...
#         self.char2id['Û'] = 3  # <unk> token
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        word_index = self.words2charindices(sents)
        
        ##size should be (max sentence length, batch size, max word length) 
        sents_padded = pad_sents_char(word_index, self.char_pad)
        return  torch.tensor(sents_padded, device=device).permute(1, 0, 2).contiguous()
        ### END YOUR CODE
